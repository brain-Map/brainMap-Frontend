...
interface MentorFormData {
  ...
  location: string;
  gender: string;
  profilePhoto: File | null; // new upload
  expertiseAreas: ExpertiseArea[];
  bio: string;
  education: Education[];
  workExperience: string;
  verificationDocs: File[]; // newly added files
...
export default function MentorEditForm() {
  ...
  const [existingVerificationDocs, setExistingVerificationDocs] = useState<any[]>([]); // files already on server
  const [deletedExistingDocIds, setDeletedExistingDocIds] = useState<string[]>([]);
  ...
  const removeExistingDoc = (docId: string) => {
    setExistingVerificationDocs(prev => prev.filter(d => d.id !== docId));
    setDeletedExistingDocIds(prev => [...prev, docId]);
  };
  ...
  useEffect(() => {
    // Fetch existing mentor profile and prefill form
    const fetchProfile = async () => {
      if (!id) return;
      try {
        const res = await axios.get(`${process.env.NEXT_PUBLIC_BACKEND_URL}/api/v1/domain-experts/${id}/profile`,
          { headers: { Authorization: `Bearer ${token}` } }
        );
        const profile = res.data?.data || res.data;
        if (!profile) return;
        // Map backend shape to formData where possible
        setFormData(prev => ({
          ...prev,
          ...
          expertiseAreas: profile.expertiseAreas || profile.expertise_areas || prev.expertiseAreas,
          bio: profile.bio || prev.bio,
          education: profile.education || prev.education,
          workExperience: profile.workExperience || profile.work_experience || prev.workExperience,
          linkedinProfile: profile.linkedinProfile || profile.linkedin_profile || prev.linkedinProfile,
        ...
        if (profile.verificationDocs && Array.isArray(profile.verificationDocs)) {
          // expected items: { id, name, url, type, size }
          setExistingVerificationDocs(profile.verificationDocs.map((d: any) => ({
        ...
        console.error('Failed fetching mentor profile', err);
      }
    };
  ...
  const handleSubmit = async (e?: React.FormEvent) => {
    ...
    try {
      ...
      // include ids of existing docs user removed so backend can delete them
      data.append('deletedDocIds', new Blob([JSON.stringify(deletedExistingDocIds)], { type: 'application/json' }));

      await axios.put(
        `${process.env.NEXT_PUBLIC_BACKEND_URL}/api/v1/domain-experts/${id}/edit`,
        data,
        { headers: { Authorization: `Bearer ${token}` } }
      );
    ...
    <div className="space-y-6">
      ...
      <div>
        <label className="block text-sm font-medium text-gray-700 mb-2">
          ...
          <span className="text-xs text-gray-500 ml-2">
            ({existingVerificationDocs.length + formData.verificationDocs.length} file{existingVerificationDocs.length + formData.verificationDocs.length !== 1 ? 's' : ''} uploaded)
          </span>
        </label>
        ...
        {/* Uploaded Files List (existing + newly added) */}
        {(existingVerificationDocs.length > 0 || formData.verificationDocs.length > 0) && (
          <div className="mt-6 space-y-3">
            <h4 className="text-sm font-medium text-gray-700 flex items-center">
              <FileText className="w-4 h-4 mr-2" />
              Uploaded Documents ({existingVerificationDocs.length + formData.verificationDocs.length})
            </h4>
            
            <div className="space-y-2 max-h-64 overflow-y-auto">
              {existingVerificationDocs.map((file, index) => (
                <div key={file.id} className="group flex items-center justify-between p-4 bg-white border border-gray-200 rounded-lg hover:shadow-sm transition-all duration-200">
                  ...
                  <div className="flex items-center gap-2">
                    <a href={file.url} target="_blank" rel="noreferrer" className="text-blue-600 text-sm">View</a>
                    <button
                      type="button"
                      onClick={() => removeExistingDoc(file.id)}
            ...
            <div className="flex items-center justify-between p-3 bg-gray-50 rounded-lg text-sm">
              <span className="text-gray-600">
                Total: {existingVerificationDocs.length + formData.verificationDocs.length} file{existingVerificationDocs.length + formData.verificationDocs.length !== 1 ? 's' : ''}
              </span>
              <span className="text-gray-600">
                Size: {formatFileSize((existingVerificationDocs.reduce((total, f) => total + (f.size || 0), 0) + formData.verificationDocs.reduce((total, f) => total + f.size, 0)))}
              </span>
            </div>
          </div>
        )}
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        <div>
          ...
          <input
            type="url"
            value={(formData as any).linkedinProfile}

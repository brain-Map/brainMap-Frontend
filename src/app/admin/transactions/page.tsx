...
export default function TransactionsPage() {
  ...
  useEffect(() => {
    ...
    setCurrentPage(1);
  }, [searchQuery, typeFilter, statusFilter, roleFilter, dateRange, transactions]);
  ...
  const handleTransactionAction = (txnId: string, action: "approve" | "reject" | "flag") => {
    setTransactions((prev) =>
      prev.map((txn) =>
        txn.id === txnId
          ? {
              ...txn,
              status: action === "approve" ? "Approved" : action === "reject" ? "Rejected" : txn.status,
              notes: action === "flag" ? (txn.notes || "") + " [FLAGGED]" : txn.notes,
            }
          : txn
      )
    );
  ...
  const quickStats = [
    ...
    },
    {
      ...
      value: new Set(transactions.map(t => t.paymentMethod).filter(Boolean)).size,
      icon: CreditCard,
              ...
              <CardContent className="p-6">
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <p className="text-2xl font-bold text-green-600">${summaryStats.totalPayments.toFixed(2)}</p>
                  </div>
              ...
              <CardContent className="p-6">
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <p className="text-2xl font-bold text-purple-600">${summaryStats.totalWithdrawals.toFixed(2)}</p>
                  </div>
              ...
              <CardContent className="p-6">
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <p className="text-2xl font-bold text-yellow-600">{summaryStats.pendingWithdrawals}</p>
                  </div>
              ...
              <CardContent className="p-6">
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <p className="text-2xl font-bold text-red-600">{summaryStats.failedTransactions}</p>
                  </div>
                  ...
                  <TableBody>
                    {currentTransactions.map((transaction) => (
                      <TableRow key={transaction.id}>
                        ...
                        <TableCell>
                          <Badge variant="outline" className={transaction.userRole === "Expert" ? "border-purple-200 text-purple-700" : "border-blue-200 text-blue-700"}>
                  ...
                  <div key={log.id} className="border rounded-lg p-4 bg-gray-50">
                    <div className="flex items-center justify-between mb-2">
                      <div className="flex items-center gap-2">
                        ...
                        <span className="font-mono text-sm text-gray-600">{log.transactionId}</span>
                      </div>
                    ...
                    <p className="text-xs text-gray-500">by {log.adminUser}</p>
                  </div>
                ))}
              </div>
            </CardContent>
